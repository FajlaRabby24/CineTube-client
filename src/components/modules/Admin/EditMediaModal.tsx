...
import { updateMedia } from "@/services/Media/mediaActions.service";
import { GenreEnum, updateMediaZodSchema } from "@/zod/media.validation";
...
export const EditMediaModal = ({
  ...
  const [updateError, setEditError] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (initialData && open) {
      setGenres(initialData.genres?.map((g: any) => g.genre) || []);
      setTags(initialData.tags?.map((t: any) => t.tag?.name || t) || []);
      setCastMembers(
        initialData.castMembers?.map((c: any) => ({
      ...
      setDirectors(
        initialData.directors?.map((d: any) => ({
  ...
  const { mutateAsync: updateMediaMutate, isPending: isUpdating } = useMutation(
    {
  ...
  const form = useForm({
    defaultValues: {
      ...
      ageRating: initialData?.ageRating || "PG_13",
      duration: initialData?.duration || 120,
      youtubeStreamUrl: initialData?.youtubeStreamUrl || "",
      imdbId: initialData?.imdbId || "",
      language: initialData?.language || "English",
      country: initialData?.country || "USA",
      pricingType:
        initialData?.pricingType || ("PREMIUM" as "FREE" | "PREMIUM"),
      status:
        initialData?.status ||
        ("PUBLISHED" as "DRAFT" | "PUBLISHED" | "UNPUBLISHED"),
      isFeatured: initialData?.isFeatured || false,
      isTrending: initialData?.isTrending || false,
      isEditorsPick: initialData?.isEditorsPick || false,
    },
    onSubmit: async ({ value }) => {
      try {
        ...
        const result = await updateMediaMutate({
          id: mediaId,
        ...
        setEditError("Failed to update media. Please try again.");
      }
    },
  });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[700px] p-0">
        <DialogHeader className="p-6 pb-2">
          ...
          <DialogDescription>Update movie or series details.</DialogDescription>
        </DialogHeader>
                ...
                <FieldGroup className="grid gap-4 sm:grid-cols-2">
                  <Field>
                    <form.Field
                      name="title"
                      validators={{
                        onChange: updateMediaZodSchema.shape.title,
                      }}
                    >
                      {(field) => (
                  ...
                  <Field>
                    <form.Field
                      name="synopsis"
                      validators={{
                        onChange: updateMediaZodSchema.shape.synopsis,
                      }}
                    >
                      {(field) => (
                        <div className="space-y-2 sm:col-span-2">
