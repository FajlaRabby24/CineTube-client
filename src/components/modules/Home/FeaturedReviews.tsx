...
const reviews = [
  {
    ...
    rating: 10,
    content: "A masterpiece of cinema. Heath Ledger's Joker is legendary. The pacing, the score, and the direction are all perfect.",
    likes: 245,
  },
  {
    id: 2,
    ...
    rating: 9,
    content: "Mind-bending concept executed perfectly. The ending still leaves me thinking weeks later. Truly spectacular filmmaking.",
    likes: 189,
  },
  {
    id: 3,
    ...
    rating: 9,
    content: "Visually stunning and emotionally powerful. Hans Zimmer's score is a character of its own. A journey through space and time.",
    likes: 156,
  },
];
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-4 text-white">
          <div className="space-y-4">
            <Badge className="bg-primary/20 text-primary border-primary/30">Audience Voice</Badge>
          ...
          <p className="text-slate-400 text-sm max-w-sm">
            Discover trending reviews from our global community of move lovers and critics.
          </p>
        </div>
              ...
              <div className="h-full p-8 rounded-[2.5rem] bg-white/5 border border-white/10 backdrop-blur-md transition-all duration-500 group-hover:bg-white/10 group-hover:border-primary/30 flex flex-col justify-between">
                <div className="space-y-6">
                   <div className="flex justify-between items-start">
                      <div className="flex gap-1">
                        {[...Array(5)].map((_, i) => (
                          <StarIcon key={i} className={`size-4 ${i < (review.rating / 2) ? "text-primary fill-primary" : "text-white/20"}`} />
                        ))}
                      </div>
                      <QuoteIcon className="size-8 text-white/10 group-hover:text-primary/20 transition-colors" />
                   </div>
                   <p className="text-slate-300 leading-relaxed italic text-lg line-clamp-4">"{review.content}"</p>
                </div>
                ...
                <div className="mt-8 flex items-center justify-between border-t border-white/5 pt-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-bold text-white font-outfit">{review.user.name}</h4>
                      <p className="text-xs text-primary font-bold uppercase tracking-widest line-clamp-1">{review.movieTitle}</p>
                    </div>
                  </div>
...
const Badge = ({ children, className }: { children: React.ReactNode; className?: string }) => (
  <div className={`inline-flex items-center rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-widest transition-colors ${className}`}>
    {children}
  </div>
);
