...
const TrendingMovies = () => {
    ...
    return (
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="flex gap-4 overflow-hidden">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="aspect-[2/3] w-[200px] shrink-0 animate-pulse rounded-xl bg-muted" />
          ))}
        </div>
      </div>
    );
  ...
  const trendingItems = (movies as unknown as { data: IMediasResponse[] })?.data || [];
      ...
      <div className="container mx-auto px-4">
        <div className="mb-8 flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" asChild className="group hover:bg-transparent">
            <Link href="/media?isTrending=true" className="flex items-center gap-1 font-semibold group-hover:text-primary transition-colors">
              Explore All <ChevronRightIcon className="size-4 group-hover:translate-x-1 transition-transform" />
            </Link>
          </Button>
        </div>
                ...
                <Link href={`/media/${media.slug}`}>
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
                    <div className="space-y-3 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                       <h3 className="font-bold text-white text-sm line-clamp-2 leading-tight">{media.title}</h3>

                       <div className="flex items-center gap-2 text-[10px] font-bold text-white/80">
                         <span className="flex items-center gap-0.5"><StarIcon className="size-2 text-yellow-400 fill-yellow-400" /> {media.averageRating}</span>
                         <span>•</span>
                       ...
                       <div className="flex gap-2">
                          ...
                          <Button size="icon" variant="outline" className="size-8 rounded-full border-white/40 text-white hover:bg-white/10">
