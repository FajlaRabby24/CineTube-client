    ...
    },
    {
      ...
      content: (
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {skillCategories.map((category) => (
              <div key={category.title} className="space-y-1.5">
                <h4 className="text-[13px] uppercase tracking-wider text-gray-500 font-bold">
                  {category.title}
                </h4>
                <div className="flex flex-wrap gap-1.5">
                  {category.skills.map((skill) => (
                    <span
                      key={skill}
                      className="text-[12px] px-2 py-0.5 rounded-md bg-white/5 border border-white/5 text-gray-300 hover:text-white hover:border-red-500/30 transition-all font-semibold"
                    >
                      {skill}
                    </span>
                  ))}
          ...
          <div className="space-y-1.5 pt-2 border-t border-white/5 flex items-center justify-between">
            <div className="space-y-1">
              <h4 className="text-[13px] uppercase tracking-wider text-orange-500 font-bold flex items-center gap-1.5">
              ...
              <div className="flex flex-wrap">
                <span className="text-[12px] px-2 py-0.5 rounded-md bg-orange-500/10 border border-orange-500/20 text-orange-400 font-semibold shadow-[0_0_8px_rgba(234,88,12,0.15)]">
                  Golang (Go)
                </span>
              </div>
            </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {[
              ...
              <span
                key={proj}
                className="text-[13px] px-2 py-1 rounded bg-red-500/10 border border-red-500/20 text-red-400 font-bold hover:scale-105 transition-transform"
              >
                {proj}
              </span>
            ))}
        ...
        return (
          ...
          >
            <MagicCard
              ...
              glowTo={card.glowTo}
              className="w-full p-4 rounded-2xl border border-white/5 bg-slate-900/40 backdrop-blur-md flex flex-col"
            >
