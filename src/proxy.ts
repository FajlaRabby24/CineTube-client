...
import {
  ...
  getUserInfo,
} from "./services/Auth/getMe.service";
...
export const proxy = async (request: NextRequest) => {
  try {
    const { pathname } = request.nextUrl; // eg: /dashboard, /profile, /login
    ...
    if (
      ...
      try {
        ...
        if (refreshed) {
          requstHeaders.set("x-token-refreshed", "1");
        }
    ...
    if (
      ...
      routerOwner === UserRole.SUPER_ADMIN ||
      routerOwner === UserRole.USER
    ) {
